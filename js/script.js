$.getJSON("data/data.json", function (data) {
  ...
  $(document).ready(function () {
    $("#tombol-input").click(function () {
      ...
      if (userInput < data.x) {
        ...
        $("#alertku").html(
          `<div class="alert alert-info" role="alert">` + data.pas + `</div>`
        );
      } else {
        $("#alertku").html(
  ...
  $("#artikel").append(
    `<div class="col-lg-4">
  <div class="card">
    <img class="card-img-top mb-2" src="https://source.unsplash.com/` +
      data["artikel"][0]["g"] +
      `/600x600" alt="card img" />
    <div class="card-body">
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h1 class="modal-title fs-5" id="artikelku-1">` +
      data["title"][0]["t"] +
      `</h1>
    ...
    <div class="card-body">
      ...
      <div class="modal fade" id="artikel-2" tabindex="-1" aria-labelledby="artikelku-2" aria-hidden="true">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h1 class="modal-title fs-5" id="artikelku-2">` +
      data["title"][1]["t"] +
      `</h1>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
    ...
    <div class="card-body">
      ...
      <div class="modal fade" id="artikel-3" tabindex="-1" aria-labelledby="artikelku-3" aria-hidden="true">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h1 class="modal-title fs-5" id="artikelku-3">` +
      data["title"][2]["t"] +
      `</h1>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
...
`
  ...
  $("#warnaku-2").attr(
    "class",
    data.warna[1]["a"] + " rounded-5 bg-opacity-25 d-inline py-1 px-4"
  );
  $("#warnaku").attr(
    "class",
    data.warna[1]["a"] + " rounded-5 bg-opacity-25 d-inline py-1 px-4"
  ...
  if (currentHour >= 4 && currentHour < 5) {
    ...
    fetch("/persiapan.html")
      .then((snap) => snap.text())
      .then((result) => {
        main.innerHTML = result;
      });
  } else if (currentHour >= 7 && currentHour < 11) {
    fetch("/aktivitas.html")
      .then((snap) => snap.text())
      .then((result) => {
        main.innerHTML = result;
      });
  } else if (currentHour >= 11 && currentHour < 13) {
    fetch("/istirahat.html")
      .then((snap) => snap.text())
      .then((result) => {
        main.innerHTML = result;
      });
